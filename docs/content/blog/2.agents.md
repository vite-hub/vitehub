...
Create an empty directory and install ViteHub Agents with the H3 server used in
this tutorial.
...
server build. Discovery and Provider Output become useful when the application
invokes Agents by name or integrates them with a host.
...
The route imports this Definition directly. The file location still keeps the
Agent visible and reviewable before any invocation starts.
...
Restart the same server and repeat the request. The response still exposes a
`text` field, but the wording now comes from the selected model.
...
Capabilities give an Agent explicit model-facing abilities such as Workspace
inspection, KV access, Blob storage, shell execution, or MCP tools. Start with
the smallest Capability set that the product needs; the first Agent does not
need any.
...
- Read [Capabilities](/docs/agents/capabilities) before exposing tools or data to a model.
