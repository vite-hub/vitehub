...
description: >-
  ...
  Sandbox, and Workspace in one Vite-only walkthrough.
date: 2026-05-28
category: Article
layout: tutorial
...
Add the database package when you want Drizzle database definitions:
...
export default defineEventHandler(() => {
  ...
  return {
    ...
    githubTokenConfigured: serverEnv.github.token.unseal().length > 0,
  }
})
```
::
...
## Definitions stay close to server code

The examples above use Vite-discovered files under `server/queues`,
`server/workflows`, `server/schedules`, `server/sandboxes`, and
`server/workspaces`. Server routes call the stable Runtime Helpers from the code
that handles product input.
...
::u-page-grid{class="pb-2"}
  ...
  :::u-page-card
  ---
  title: A better way to build AI agents for any cloud
  description: Use any AI model, flexible plugins, and a secure workspace.
