...
Create an empty directory and install the ViteHub KV package with the small
server used in this tutorial.
...
Register `hubKv()` in Vite and select the `fs-lite` driver explicitly. ViteHub
stores local values under `.data/kv`, while server code only sees the `kv`
Runtime Helper.
...
`.data/kv`. Delete that directory whenever you want to reset the local store.
...
3. `hubKv()` resolves the concrete store during the Vite build.
...
- Build [your first Agent](/blog/agents) when the product needs an actor that can invoke models, coding harnesses, or application-owned logic.
