...
export default defineAgent({
  ...
  capabilities: [workspaceShell({ commands: ['git'], timeout: 30_000 })],
})
```

Command entries accept simple executable names or absolute paths, never shell command strings. `commands: 'all'` permits any executable reachable by the Workspace Session and should be limited to a trusted host. Read mode does not commit command changes; write mode commits successful changes through Workspace rules.
...
| `commands` | `string[] \| "all"` | - | Adds an executable allowlist or unrestricted executable selection. |
