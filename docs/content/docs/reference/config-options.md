...
| Console | `boolean \| ConsoleOptions` | `console` config key in `vitehub()` | Omission or `false` registers no Console page, API handler, plugin, or assets. `true` mounts the complete read-only [Console](/docs/development/console) during development. Production builds require `{ access: 'auth' }` with callback-backed Auth policies for `/_vitehub/**` and `/api/_vitehub/console/**`, or `{ exposure: 'host-managed' }` to acknowledge equivalent host middleware. The Agent Console uses the shared invocation journal configured on discovered Agent Definitions. Without one, it falls back to local SQLite at `.vitehub/data/console.sqlite`. |
