...
| Email | `EmailVitePluginOptions` | `hubEmail(options)` | `driver` selects an exact `unemail/driver/*` subpath and `options` accepts runtime Env declarations. Use `projectRoot` only with the advanced discovered Definition path. |
