...
| --- | --- | --- | --- |
| `vite-hub` | `ViteHubOptions` | `vitehub({ preset })` in Vite `plugins` | Composes Env and includes Blob when the preset supplies a built-in store. Agent, Auth, Database, Email, KV, Queue, Rate Limit, Sandbox, Schedule, Workflow, and Workspace are enabled with `true` or explicit options. |
...
The root `vitehub()` facade enables Agent, Database, Email, KV, Queue, Rate Limit, Sandbox, Schedule, Workflow, and Workspace with `true`; direct owner-package integrations retain their detailed option types. Auth follows the same opt-in shape but currently has no plugin option bag.
