...
| --- | --- | --- | --- |
| `vite-hub` | `ViteHubOptions` | `vitehub({ preset })` in Vite `plugins` | Composes Agent, Database, Env, Workflow, and Workspace unless a key is `false`; includes Blob when the preset supplies a built-in store. Auth, Email, KV, Queue, Rate Limit, Sandbox, and Schedule are enabled with `true`. |
