...
| Email | `EmailVitePluginOptions` | `email` config key or `hubEmail(options)` | `driver` is required for explicit options and selects an exact `unemail/driver/*` subpath; `options` accepts serializable literals and runtime Env declarations. Omission disables Email in `vitehub()`. The root facade additionally accepts `email: true` on Cloudflare and rejects it on other presets; file discovery is not supported. |
