...
| Console | `boolean` | `console` config key in `vitehub()` | Omission or `false` disables the Agent invocation console. `true` mounts the complete read-only console at `/_vitehub`, its API at `/api/_vitehub/console/**`, and a fallback SQLite invocation journal at `.vitehub/data/console.sqlite` in development and production. The console does not add authorization; the host must protect these routes when their metadata is private. |
