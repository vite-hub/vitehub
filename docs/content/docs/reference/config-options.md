...
| Channels | Agent Channel options | `defineAgent({ channels })` | An Agent Channel with `global: true` is available to server code through `useChannel(name)`. ViteHub discovers it through the Agent Definition. The separate `server/channels` convention is removed. |
