...
| Browser | `BrowserModuleOptions` | `browser` config key or `hubBrowser(options)` | Omission or `false` disables Browser in `vitehub()`; `true` enables Cloudflare Browser Run with binding `BROWSER`, and `{ binding }` changes the binding name. Browser Definitions currently require the Cloudflare preset. The root integration generates the binding and Nitro compatibility flags; direct standalone `hubBrowser()` Provider Output leaves non-Browser Wrangler fields under application ownership. |
