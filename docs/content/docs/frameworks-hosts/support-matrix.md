...
| --- | --- | --- | --- | --- | --- |
| Local Vite | **Available** — active package integrations expose their stable package imports and generated registries. | **Available** — Blob `fs`, KV `fs-lite`, and Workspace `local` or `memory` cover common local state. | **Not provided** as a Local Vite target. An explicit or inferred hosted provider can still generate that provider's output during a local build. | **Not provided** | **Contract-tested** through package and docs CI; this row makes no hosted-runtime claim. |
| Cloudflare | **Package-specific** — the live primitive playground covers Blob, Database, KV, Queue, Sandbox, Schedule, Workflow, and Workspace; Agent routes and state have separate package output. | **Package-specific** — pull requests build Cloudflare output and run the primitive playground locally. | **Package-specific** — enabled integrations compose a Worker, `wrangler.json`, bindings, callbacks, and runtime modules. | **Package-specific** — R2 buckets, D1 databases, and Cloudflare Queues. | **Contract-tested**, followed by a Local Provider Run in CI. A nightly Live Smoke deploys the eight named primitives; it does not currently prove Agent routes. |
...
| Node and self-hosted | **Package-specific** — stable Runtime Helpers and server handlers run when their selected driver supports Node. | **Local-only** — Blob `fs`, KV `fs-lite`, and Workspace `local` or `memory` do not provide cross-instance durability; configure a durable provider explicitly for production state. | **Not provided** — ViteHub does not emit one unified Node deployment bundle. | **Not provided** | **Contract-tested** per package. **Live proof not published** as one self-hosted deployment suite. |
