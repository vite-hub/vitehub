...
| Cloudflare | **Package-specific** — the live primitive playground covers Blob, Database, KV, Queue, Rate Limit, Sandbox, Schedule, Workflow, and Workspace; Agent routes have separate package output. | **Package-specific** — pull requests build Cloudflare output and run the primitive playground locally. | **Package-specific** — enabled integrations compose a Worker, `wrangler.json`, Rate Limiting bindings, callbacks, and runtime modules. | **Package-specific** — R2 buckets, D1 databases, and Cloudflare Queues. Rate Limiting bindings require no separate provisioned resource. | **Contract-tested**, followed by a Local Provider Run in CI. A nightly Live Smoke deploys the nine named primitives; it does not currently prove Agent routes. |
...
- Provider Output contracts: the `vite-output` or `provider-output` tests in the Blob, Database, KV, Queue, Rate Limit, Schedule, and Workflow packages, plus Agent provider tests.
