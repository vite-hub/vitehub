...
The Agent's Box defines where the process runs. Use an isolation-capable provider Box for isolated execution or `"trusted-host"` when the Agent intentionally runs with the host service account's authority. Workspace never selects an execution runtime.
