...
Rich session replay requires a trace log created with `{ content: "content" }`. The default metadata-only trace policy records activity milestones but strips prompts, message text, tool input, and tool output. Agent Invocation journals bound each content string to 64 KiB and each metadata string to 512 characters. They also bound values to 32 collection items, four nesting levels, and 256 observations per invocation.
...
Pass `selectedId` and handle `select` to connect the list to the current route. Pass `hasMore`, `loading`, and handle `endReached` when the application lazy-loads another page. When a page fails, change `retryKey` from a retry action to request the same page again, including when the list cannot scroll.
