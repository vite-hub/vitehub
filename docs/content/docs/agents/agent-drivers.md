...
Codex `credentials` accepts the complete `auth.json` JSON string, a sealed Server Env value, or an invocation-time resolver. ViteHub writes it to a private temporary shadow home and removes that home after the provider runtime stops. The primary Codex home continues to own session state; credentials never enter the Agent Workspace. One ViteHub Provider Driver host process must own that durable home. Replicas or separate processes must configure distinct `providerSettings.homePath` values because sharing a durable Codex home across processes is unsupported. ViteHub coordinates credential-overlay access only within one host process. Do not combine `credentials` with `providerSettings.shadowHomePath` because both own the same authentication boundary.
