...
export default telemetry.plugin
```
...
HTTP request log export defaults to `logs: 'failures'`, which exports request records with a warning or error level or a status of 400 or higher. Set `logs: 'all'` to retain successful request logs or `logs: false` to disable HTTP request log export. Request records are identified by evlog's `DrainContext.request` metadata, which the Nitro integration supplies. Custom hosts should supply this metadata when calling `telemetry.drain` for HTTP requests; `method` and `path` fields on the event alone do not identify a request. Other application logs, Agent events and exception delivery are unaffected.
...
const telemetry = createAgentEvlog({
  ...
  papercuts: {
    async invocations() {
      ...
      return getConsoleInvocations()
    },
  },
})

// Add telemetry.capability to the Agent and telemetry.plugin to Nitro.
// The integration registers the tool, persists reports and owns replay/shutdown.
```
