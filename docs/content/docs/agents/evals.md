...
Keep the harness adapter, credentials, session key, and runtime selection on the Agent Definition. An eval should declare scenarios and scorers; it should not duplicate the Agent Driver setup. Harness sandbox provider setup is Agent Package runtime plumbing resolved from defaults or integration configuration. `sandbox()` remains the Capability for model-facing command execution authority.
