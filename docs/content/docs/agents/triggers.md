...
Setting `busy: 'steer'` first offers the full Agent input to an active inline Harness invocation with the same scoped concurrency key. An accepted input is deduplicated by delivery ID; when no matching prompt control accepts it, the delivery follows the same durable queue path.
