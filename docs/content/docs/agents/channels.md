...
Channel history export archives inline data, size-declared adapter-owned `fetchData`, Blob data, and HTTPS URL references within a 25 MiB total attachment budget. Lazy adapter reads need a trustworthy non-negative `size` within the remaining budget. The export stops waiting for each provider history read, URL read, or adapter-owned read after 30 seconds or when its request is aborted; URL downloads also receive the abort signal, reject credentials, and reject redirects. The Chat SDK history and `fetchData` contracts have no cancellation channel, so their underlying private I/O remains adapter-owned and may settle after the export stops waiting. The adapter also owns host trust: validate attachment URLs against an application-owned allowlist or fetch proxy before persisting them. Attachments that exceed the remaining budget, contain malformed retained data, fail rehydration, omit the size required for a lazy read, or otherwise cannot be read remain in `history.json` as unavailable references.
