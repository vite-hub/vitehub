...
Channel history export archives inline data, adapter-owned `fetchData`, Blob data, and HTTPS URL references within a 25 MiB total attachment budget. URL downloads reject credentials and redirects, stop after 30 seconds, and also stop when the export request is aborted. The adapter still owns host trust: validate attachment URLs against an application-owned allowlist or fetch proxy before persisting them. Attachments that exceed the remaining budget or cannot be read remain in `history.json` as unavailable references.
