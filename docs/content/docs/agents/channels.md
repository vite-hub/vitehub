...
When finish hooks own an adapter-backed Channel reply through `messages: { delivery: 'manual' }`, ViteHub uses the generated Agent Workflow when one is active and the invocation can cross its durable boundary. Set `durable: true` to require that handoff or `durable: false` to keep execution inline. Configured overlap policies such as `serial`, `drop`, `queue`, and `reject` stay inline because they retain webhook ownership until the invocation settles; they cannot be combined with `durable: true`. ViteHub materializes lazy attachments, acknowledges the webhook after starting the Workflow, and lets `event.reply()` deliver through the original Channel and thread after the Workflow finishes. Built-in Blob and Database primitives are reconstructed inside the Workflow, while other host Capability handles remain inline in auto mode and make required handoff fail instead of silently falling back. Only the request URL crosses this boundary: webhook headers, bodies, and the provider request deadline remain process-local, while an explicit `messages.timeout` remains the Agent Invocation budget. Durable delivery does not stream progress; provide an `agent:error` reply when the Channel must surface deferred failures.
