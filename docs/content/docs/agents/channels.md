...
Channel history export archives inline data, size-declared adapter-owned `fetchData`, Blob data, and HTTPS URL references within a 25 MiB total attachment budget. Lazy adapter reads need a trustworthy non-negative `size` within the remaining budget. The export stops waiting for URL and adapter-owned reads after 30 seconds or when its request is aborted; URL downloads also receive the abort signal, reject credentials, and reject redirects. The Chat SDK `fetchData` contract has no cancellation channel, so its underlying private I/O remains adapter-owned and may settle after the export stops waiting. The adapter also owns host trust: validate attachment URLs against an application-owned allowlist or fetch proxy before persisting them. Attachments that exceed the remaining budget, fail rehydration, omit the size required for a lazy read, or otherwise cannot be read remain in `history.json` as unavailable references.
