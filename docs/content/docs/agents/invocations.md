...
Provider Agent tools require an input schema that can produce JSON Schema. Use a JSON Schema object or a Standard Schema implementation with `~standard.jsonSchema.input`; a plain Valibot schema has validation but lacks that JSON Schema method. Keep additional validation inside `execute` when using a JSON Schema object.
