...
The driver subpath selects the upstream Unemail provider. The Env declaration is serialized, but its value is resolved in the server runtime for every send, so build output never contains the API key and request-scoped Cloudflare secrets stay current.
...
Keep credentials in Server Env or the deployment platform's secret store. The canonical configuration accepts Env declarations, never resolved credential strings.
...
| `options` | `EnvRuntimeConfigOptions` | `{}` | Supplies serializable literals and runtime Env declarations. Values resolve in the server runtime for every send. |
...
Configure either `driver` and `options` or one discovered Email Definition, never both. The integration serializes only the driver subpath and Env declarations into a server-only virtual module; credential values remain in the runtime environment.
