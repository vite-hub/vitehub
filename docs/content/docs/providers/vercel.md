...
`@vite-hub/kv` supports a Vercel path through the Upstash-backed `upstash` driver.
...
`@vite-hub/database` supports Vercel on Vite through hosted libSQL connections, including optional named databases behind the same runtime surface.
...
`@vite-hub/blob` supports a Vercel path on Vite and Nitro through the `vercel-blob` driver.
...
`@vite-hub/queue` supports a Vercel path on Vite and Nitro through `queue.provider = 'vercel'`.
...
`@vite-hub/schedule` supports Vercel Provider Output on Vite and Nitro when the Vite Integration or Nitro Integration discovers Static Schedule Definitions.
