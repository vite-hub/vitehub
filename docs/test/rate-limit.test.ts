...
describe("Rate Limit documentation", () => {
  it("teaches the canonical first success and owner-package escape hatch", () => {
    ...
    expect(primitive).toContain("const uploads = defineRateLimit('image-upload'")
    expect(primitive).toContain("uploads.consume()")
    expect(primitive).toContain("defaults to the client address installed by the active host integration")
    expect(primitive).toContain("if (!decision.allowed)")
    expect(primitive).toContain("does not need a default export, dedicated directory, or file suffix")
  ...
  it("keeps guarantees, identity, and backend limitations explicit", () => {
    ...
    expect(primitive).toContain("Identity and HTTP rejection remain application policy")
    ...
    expect(primitive).toContain("application code should keep using the handle")
