...
The consumer migration is complete when it works without the patch. Retire combined patches hunk by hunk as their fixes land upstream. If release or publication is outside the task’s authority, hand off the verified source fix, remaining release dependency, and exact repin or patch-removal step; do not claim the consumer migration is complete.
...
- Before editing, name the affected contract variants and trace the changed value through its owning path: definition or configuration, generated output, runtime, and consumer. Verify the variants whose behavior can change. Reuse evidence only when variants exercise the same changed path; name distinct unverified variants. Once relevant checks and required CI pass, broaden or repeat verification only for a new change, failure, or unresolved concern.
