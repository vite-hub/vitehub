...
Pullfrog is review evidence, not an optional check. If Pullfrog appears on the pull request, its latest linked workflow run must complete successfully and it must submit a review for the expected head. While the run is queued or running, stop unchanged. A failed or cancelled run, or a review for another head, blocks the merge.
