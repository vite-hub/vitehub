...
const liveOnlyMessage = "Blob e2e requires a deployed app: vp run blob:e2e --mode live --url <url>";
const log = (message) => console.log(`[blob e2e] ${message}`);
const vercelProtectionBypass = process.env.VERCEL_PROTECTION_BYPASS;

async function assertBlobRoundtrip(baseURL) {
  ...
  const list = await ofetch("/api/blob", { baseURL, headers });
  ...
  const served = await fetch(
    new URL(`/api/blob/serve?pathname=${encodeURIComponent(pathname)}`, baseURL),
    { headers },
  );
