  ...
  return {
    async close() {
      ...
      const cleanup = await sandbox.run({
        command: `codex_home="$HOME/.vitehub/codex-home-${invocation.id}" && ambient_home="\${VITEHUB_AMBIENT_CODEX_HOME:-$HOME/.codex}" && baseline="$codex_home/.vitehub-baseline" && status=0; rm -rf -- "$codex_home/skills" "$codex_home/skills.vitehub-managed" || status=$?; for state in auth.json config.toml; do if [ -f "$baseline/$state" ] && cmp -s "$baseline/$state" "$codex_home/$state"; then rm -f -- "$codex_home/$state" || status=$?; fi; done; rm -rf -- "$baseline" || status=$?; if [ "$status" -eq 0 ]; then mkdir -p "$ambient_home" || status=$?; fi; if [ "$status" -eq 0 ]; then cp -R "$codex_home"/. "$ambient_home" || status=$?; fi; if [ "$status" -eq 0 ]; then rm -rf -- "$codex_home" || status=$?; fi; exit "$status"`,
      })
