...
async function prepareCodexHome(session: object, invocation?: { id?: string, isolateBoxHome: boolean }): Promise<{ close: () => Promise<void> } | undefined> {
  ...
  const result = await sandbox.run({
    command:
      'codex_home="${CODEX_HOME:-$HOME/.codex}" && ambient_home="${VITEHUB_AMBIENT_CODEX_HOME:-$HOME/.codex}" && mkdir -p "$codex_home" && chmod 700 "$codex_home" && if [ "$codex_home" != "$ambient_home" ] && [ -d "$ambient_home" ]; then cp -R "$ambient_home"/. "$codex_home"; fi && rm -rf -- "$codex_home/skills/.git" && manifest="$codex_home/skills.vitehub-managed" && if [ -f "$manifest" ]; then while IFS= read -r managed || [ -n "$managed" ]; do case "$managed" in \'\'|*/*|.. ) exit 1 ;; esac; rm -rf -- "$codex_home/skills/$managed"; done < "$manifest" && rm -f -- "$manifest"; fi && if [ ! -e "$codex_home/config.toml" ]; then : > "$codex_home/config.toml"; fi && chmod 600 "$codex_home/config.toml" && if [ "$codex_home" != "$ambient_home" ]; then baseline="$codex_home.vitehub-baseline" && rm -rf -- "$baseline" && mkdir -p "$baseline" && cp -R "$codex_home"/. "$baseline"; fi',
  })
