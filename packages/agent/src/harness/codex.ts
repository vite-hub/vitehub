  ...
  return {
    async close() {
      ...
      const cleanup = await sandbox.run({
        command: `codex_home="$HOME/.vitehub/codex-home-${invocation.id}" && ambient_home="\${VITEHUB_AMBIENT_CODEX_HOME:-$HOME/.codex}" && baseline="$codex_home.vitehub-baseline" && status=0; rm -rf -- "$codex_home/skills/.git" || status=$?; manifest="$codex_home/skills.vitehub-managed"; if [ -f "$manifest" ]; then while IFS= read -r encoded || [ -n "$encoded" ]; do managed=$(printf '%s' "$encoded" | base64 -d) || { status=1; break; }; case "$managed" in ''|/*|..|../*|*/..|*/../*) status=1; break ;; esac; rm -rf -- "$codex_home/skills/$managed" || status=$?; done < "$manifest"; rm -f -- "$manifest" || status=$?; fi; manifest="$codex_home/skills/.vitehub-colocated"; if [ -f "$manifest" ]; then while IFS= read -r managed || [ -n "$managed" ]; do case "$managed" in ''|*/*|.. ) status=1; break ;; esac; rm -rf -- "$codex_home/skills/$managed" || status=$?; done < "$manifest"; rm -f -- "$manifest" || status=$?; fi; if [ "$status" -eq 0 ] && [ -d "$baseline" ]; then find "$baseline" -type f -exec sh -c 'baseline="$1"; codex_home="$2"; ambient_home="$3"; shift 3; for seeded do relative="\${seeded#"$baseline"/}"; if [ ! -e "$codex_home/$relative" ]; then if cmp -s "$seeded" "$ambient_home/$relative"; then rm -f -- "$ambient_home/$relative" || exit $?; fi; elif cmp -s "$seeded" "$codex_home/$relative"; then rm -f -- "$codex_home/$relative" || exit $?; fi; done' sh "$baseline" "$codex_home" "$ambient_home" {} + || status=$?; fi; if [ "$status" -eq 0 ]; then rm -rf -- "$baseline" || status=$?; fi; if [ "$status" -eq 0 ]; then mkdir -p "$ambient_home" || status=$?; fi; if [ "$status" -eq 0 ]; then cp -R "$codex_home"/. "$ambient_home" || status=$?; fi; if [ "$status" -eq 0 ]; then rm -rf -- "$codex_home" || status=$?; fi; exit "$status"`,
      })
