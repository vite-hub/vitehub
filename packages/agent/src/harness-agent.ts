...
async function prepareHarnessColocatedSkills(
  ...
  const cleanupCommand = refresh
    ? `if [ -L ${target} ]; then printf '%s\\n' 'Persisted Skill directory cannot be a symlink.' >&2; exit 1; fi; oldLegacy=${target}/.vitehub-colocated; oldManifest=${target}/.vitehub-colocated-v2; manifestBase=${colocatedManifest}; manifest=; for candidate in "$manifestBase" "$manifestBase".[0-9]*; do if [ -f "$candidate" ] && [ "$(sed -n '1p' "$candidate")" = '${colocatedManifestMarker}' ]; then manifest="$candidate"; break; fi; done; if [ -z "$manifest" ]; then manifest="$manifestBase"; manifestIndex=0; while [ -e "$manifest" ]; do manifestIndex=$((manifestIndex + 1)); manifest="$manifestBase.$manifestIndex"; done; fi; if [ -f "$oldLegacy" ]; then rm -f -- "$oldLegacy" || exit $?; fi; if [ -f "$oldManifest" ]; then printf '%s\\n' '${colocatedManifestMarker}' > "$manifest" || exit $?; cat "$oldManifest" >> "$manifest" || exit $?; rm -f -- "$oldManifest" || exit $?; fi; if [ -f "$manifest" ]; then tail -n +2 "$manifest" | while IFS= read -r encoded || [ -n "$encoded" ]; do managed=$(printf '%s' "$encoded" | base64 -d && printf .) || exit 1; managed=\${managed%.}; case "$managed" in ''|*/*|.. ) exit 1 ;; esac; if [ ! -L ${target}/"$managed" ]; then chmod -R u+w -- ${target}/"$managed" 2>/dev/null || true; fi; rm -rf -- ${target}/"$managed" || exit $?; done || exit $?; fi && rm -f -- "$manifest" || exit $?`
    : undefined
  if (!workspace) {
