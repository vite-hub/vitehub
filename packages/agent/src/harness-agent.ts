...
async function prepareHarnessGlobalSkills(
  ...
  const install = await (session as HarnessGlobalSkillsSandbox).run({
    abortSignal,
    ...(workingDirectory ? { workingDirectory } : {}),
    command: `rm -rf -- ${quotedStagingDirectory}/.git && find ${quotedStagingDirectory} -type f -path "*/scripts/*" -exec chmod +x {} + && : > ${quotedManagedManifest} && for encoded in ${encodedSkillDirectories}; do managed=$(printf '%s' "$encoded" | base64 -d && printf .) || exit 1; managed=\${managed%.}; case "$managed" in ''|/*|..|../*|*/..|*/../*) printf '%s\\n' 'Invalid ViteHub-managed Skill path.' >&2; exit 1 ;; esac; remainder=$managed; current=${quotedDirectory}; relative=; owned=; while [ -n "$remainder" ]; do component=\${remainder%%/*}; relative=\${relative:+$relative/}$component; current="$current/$component"; if [ -L "$current" ]; then printf '%s\\n' 'ViteHub-managed Skill path cannot traverse a symlink.' >&2; exit 1; fi; if [ -z "$owned" ] && [ ! -e "$current" ]; then owned=$relative; fi; if [ "$remainder" = "\${remainder#*/}" ]; then break; fi; remainder=\${remainder#*/}; done; if [ -n "$owned" ]; then parent=\${managed%/*}; if [ "$parent" != "$managed" ]; then mkdir -p -- ${quotedDirectory}/"$parent" || exit $?; fi; cp -R -- ${quotedStagingDirectory}/"$managed" ${quotedDirectory}/"$managed" || exit $?; encodedOwned=$(printf '%s' "$owned" | base64) || exit $?; encodedOwned=$(printf '%s' "$encodedOwned" | tr -d '\\n') || exit $?; printf '%s\\n' "$encodedOwned" >> ${quotedManagedManifest} || exit $?; fi; done && rm -rf -- ${quotedStagingDirectory}`,
  })
