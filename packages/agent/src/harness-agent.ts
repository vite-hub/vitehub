...
async function prepareHarnessGlobalSkills(
  ...
  const managedParentsManifest = `${directory}.vitehub-managed-parents`
  const quotedManagedManifest = `'${managedManifest.replace(/'/g, "'\\''")}'`
  const quotedManagedParentsManifest = `'${managedParentsManifest.replace(/'/g, "'\\''")}'`
  const managedParentsManifestMarker = "vitehub-managed-skill-parents-v1"
  const selectManagedParentsManifest = `managedParentsManifestBase=${quotedManagedParentsManifest}; managedParentsManifest=; unreadableParentsManifest=; for candidate in "$managedParentsManifestBase" "$managedParentsManifestBase".[0-9]*; do if [ -f "$candidate" ]; then if [ ! -r "$candidate" ]; then unreadableParentsManifest=1; continue; fi; candidateMarker=$(sed -n '1p' "$candidate") || exit $?; if [ "$candidateMarker" = '${managedParentsManifestMarker}' ]; then managedParentsManifest="$candidate"; break; fi; fi; done; if [ -z "$managedParentsManifest" ] && [ -n "$unreadableParentsManifest" ]; then printf '%s\\n' 'Persisted Skill parent ownership manifest cannot be read.' >&2; exit 1; fi; if [ -z "$managedParentsManifest" ]; then managedParentsManifest="$managedParentsManifestBase"; managedParentsManifestIndex=0; while [ -e "$managedParentsManifest" ]; do managedParentsManifestIndex=$((managedParentsManifestIndex + 1)); managedParentsManifest="$managedParentsManifestBase.$managedParentsManifestIndex"; done; fi`
  const ensure = await (session as HarnessGlobalSkillsSandbox).run({
    abortSignal,
    ...(workingDirectory ? { workingDirectory } : {}),
    command: `if [ -L ${quotedDirectory} ]; then printf '%s\\n' 'Global Skill directory cannot be a symlink.' >&2; exit 1; fi && mkdir -p -- ${quotedDirectory} && ${selectManagedParentsManifest} && if [ -f ${quotedManagedManifest} ]; then while IFS= read -r encoded || [ -n "$encoded" ]; do managed=$(printf '%s' "$encoded" | base64 -d && printf .) || exit 1; managed=\${managed%.}; case "$managed" in ''|/*|..|../*|*/..|*/../*) printf '%s\\n' 'Invalid ViteHub-managed Skill path.' >&2; exit 1 ;; esac; remainder=$managed; current=${quotedDirectory}; while [ "$remainder" != "\${remainder#*/}" ]; do component=\${remainder%%/*}; current="$current/$component"; if [ -L "$current" ]; then printf '%s\\n' 'ViteHub-managed Skill path cannot traverse a symlink.' >&2; exit 1; fi; remainder=\${remainder#*/}; done; if [ ! -L ${quotedDirectory}/"$managed" ]; then chmod -R u+rwX -- ${quotedDirectory}/"$managed" 2>/dev/null || true; fi; rm -rf -- ${quotedDirectory}/"$managed" || exit $?; done < ${quotedManagedManifest}; fi && if [ -f "$managedParentsManifest" ]; then tail -n +2 "$managedParentsManifest" | awk '{ line[NR]=$0 } END { for (i=NR;i>0;i--) print line[i] }' | while IFS= read -r encoded || [ -n "$encoded" ]; do parent=$(printf '%s' "$encoded" | base64 -d && printf .) || exit 1; parent=\${parent%.}; case "$parent" in ''|/*|..|../*|*/..|*/../*) printf '%s\\n' 'Invalid ViteHub-managed Skill parent path.' >&2; exit 1 ;; esac; remainder=$parent; current=${quotedDirectory}; while [ -n "$remainder" ]; do component=\${remainder%%/*}; current="$current/$component"; if [ -L "$current" ]; then printf '%s\\n' 'ViteHub-managed Skill parent cannot traverse a symlink.' >&2; exit 1; fi; if [ "$remainder" = "\${remainder#*/}" ]; then break; fi; remainder=\${remainder#*/}; done; rmdir -- ${quotedDirectory}/"$parent" 2>/dev/null || true; done || exit $?; fi && rm -f -- ${quotedManagedManifest} "$managedParentsManifest"`,
  })
  ...
  const install = await (session as HarnessGlobalSkillsSandbox).run({
    abortSignal,
    ...(workingDirectory ? { workingDirectory } : {}),
    command: `${selectManagedParentsManifest} && rm -rf -- ${quotedStagingDirectory}/.git && find ${quotedStagingDirectory} -type f -path "*/scripts/*" -exec chmod +x {} + && : > ${quotedManagedManifest} && printf '%s\\n' '${managedParentsManifestMarker}' > "$managedParentsManifest" && for encoded in ${encodedSkillDirectories}; do managed=$(printf '%s' "$encoded" | base64 -d && printf .) || exit 1; managed=\${managed%.}; case "$managed" in ''|/*|..|../*|*/..|*/../*) printf '%s\\n' 'Invalid ViteHub-managed Skill path.' >&2; exit 1 ;; esac; remainder=$managed; current=${quotedDirectory}; relative=; while [ "$remainder" != "\${remainder#*/}" ]; do component=\${remainder%%/*}; relative=\${relative:+$relative/}$component; current="$current/$component"; if [ -L "$current" ]; then printf '%s\\n' 'ViteHub-managed Skill path cannot traverse a symlink.' >&2; exit 1; fi; if [ ! -e "$current" ]; then encodedParent=$(printf '%s' "$relative" | base64) || exit $?; encodedParent=$(printf '%s' "$encodedParent" | tr -d '\\n') || exit $?; printf '%s\\n' "$encodedParent" >> "$managedParentsManifest" || exit $?; fi; remainder=\${remainder#*/}; done; if [ ! -e ${quotedDirectory}/"$managed" ] && [ ! -L ${quotedDirectory}/"$managed" ]; then parent=\${managed%/*}; if [ "$parent" != "$managed" ]; then mkdir -p -- ${quotedDirectory}/"$parent" || exit $?; fi; printf '%s\\n' "$encoded" >> ${quotedManagedManifest} || exit $?; cp -R -- ${quotedStagingDirectory}/"$managed" ${quotedDirectory}/"$managed" || exit $?; fi; done && rm -rf -- ${quotedStagingDirectory}`,
  })
