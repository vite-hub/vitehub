...
async function prepareHarnessGlobalSkills(
  ...
  const managedMetadataDirectory = `${directory}.vitehub`
  const managedParentsManifest = `${managedMetadataDirectory}/managed-skill-parents`
  const quotedManagedManifest = `'${managedManifest.replace(/'/g, "'\\''")}'`
  const quotedManagedMetadataDirectory = `'${managedMetadataDirectory.replace(/'/g, "'\\''")}'`
