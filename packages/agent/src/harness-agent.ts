...
async function prepareHarnessColocatedSkills(
  ...
  const cleanupCommand = refresh
    ? `if [ -L ${target} ]; then printf '%s\\n' 'Persisted Skill directory cannot be a symlink.' >&2; exit 1; fi; oldLegacy=${target}/.vitehub-colocated; oldManifest=${target}/.vitehub-colocated-v2; manifestBase=${colocatedManifest}; manifest=; for candidate in "$manifestBase" "$manifestBase".[0-9]*; do if [ -f "$candidate" ]; then if [ ! -r "$candidate" ]; then printf '%s\\n' 'Persisted Skill ownership manifest cannot be read.' >&2; exit 1; fi; candidateMarker=$(sed -n '1p' "$candidate") || exit $?; if [ "$candidateMarker" = '${colocatedManifestMarker}' ]; then manifest="$candidate"; break; fi; fi; done; if [ -z "$manifest" ]; then manifest="$manifestBase"; manifestIndex=0; while [ -e "$manifest" ]; do manifestIndex=$((manifestIndex + 1)); manifest="$manifestBase.$manifestIndex"; done; fi; if [ -f "$oldLegacy" ]; then rm -f -- "$oldLegacy" || exit $?; fi; if [ -f "$oldManifest" ]; then printf '%s\\n' '${colocatedManifestMarker}' > "$manifest" || exit $?; cat "$oldManifest" >> "$manifest" || exit $?; rm -f -- "$oldManifest" || exit $?; fi; if [ -f "$manifest" ]; then tail -n +2 "$manifest" | while IFS= read -r encoded || [ -n "$encoded" ]; do managed=$(printf '%s' "$encoded" | base64 -d && printf .) || exit 1; managed=\${managed%.}; case "$managed" in ''|*/*|.. ) exit 1 ;; esac; if [ ! -L ${target}/"$managed" ]; then chmod -R u+w -- ${target}/"$managed" 2>/dev/null || true; fi; rm -rf -- ${target}/"$managed" || exit $?; done || exit $?; fi && rm -f -- "$manifest" || exit $?`
    : undefined
  if (!workspace) {
...
async function prepareHarnessGlobalSkills(
  ...
  const ensure = await (session as HarnessGlobalSkillsSandbox).run({
    ...
    command: `if [ -L ${quotedDirectory} ]; then printf '%s\\n' 'Global Skill directory cannot be a symlink.' >&2; exit 1; fi && mkdir -p -- ${quotedDirectory} && if [ -f ${quotedManagedManifest} ]; then while IFS= read -r encoded || [ -n "$encoded" ]; do managed=$(printf '%s' "$encoded" | base64 -d && printf .) || exit 1; managed=\${managed%.}; case "$managed" in ''|/*|..|../*|*/..|*/../*) printf '%s\\n' 'Invalid ViteHub-managed Skill path.' >&2; exit 1 ;; esac; remainder=$managed; current=${quotedDirectory}; while [ "$remainder" != "\${remainder#*/}" ]; do component=\${remainder%%/*}; current="$current/$component"; if [ -L "$current" ]; then printf '%s\\n' 'ViteHub-managed Skill path cannot traverse a symlink.' >&2; exit 1; fi; remainder=\${remainder#*/}; done; if [ ! -L ${quotedDirectory}/"$managed" ]; then chmod -R u+w -- ${quotedDirectory}/"$managed" 2>/dev/null || true; fi; rm -rf -- ${quotedDirectory}/"$managed" || exit $?; done < ${quotedManagedManifest}; fi && if [ -f ${quotedManagedParentsManifest} ]; then awk '{ line[NR]=$0 } END { for (i=NR;i>0;i--) print line[i] }' ${quotedManagedParentsManifest} | while IFS= read -r encoded || [ -n "$encoded" ]; do parent=$(printf '%s' "$encoded" | base64 -d && printf .) || exit 1; parent=\${parent%.}; case "$parent" in ''|/*|..|../*|*/..|*/../*) printf '%s\\n' 'Invalid ViteHub-managed Skill parent path.' >&2; exit 1 ;; esac; remainder=$parent; current=${quotedDirectory}; while [ -n "$remainder" ]; do component=\${remainder%%/*}; current="$current/$component"; if [ -L "$current" ]; then printf '%s\\n' 'ViteHub-managed Skill parent cannot traverse a symlink.' >&2; exit 1; fi; if [ "$remainder" = "\${remainder#*/}" ]; then break; fi; remainder=\${remainder#*/}; done; rmdir -- ${quotedDirectory}/"$parent" 2>/dev/null || true; done || exit $?; fi && rm -f -- ${quotedManagedManifest} ${quotedManagedParentsManifest}`,
  })
  ...
  const install = await (session as HarnessGlobalSkillsSandbox).run({
    abortSignal,
    ...(workingDirectory ? { workingDirectory } : {}),
    command: `rm -rf -- ${quotedStagingDirectory}/.git && find ${quotedStagingDirectory} -type f -path "*/scripts/*" -exec chmod +x {} + && : > ${quotedManagedManifest} && : > ${quotedManagedParentsManifest} && for encoded in ${encodedSkillDirectories}; do managed=$(printf '%s' "$encoded" | base64 -d && printf .) || exit 1; managed=\${managed%.}; case "$managed" in ''|/*|..|../*|*/..|*/../*) printf '%s\\n' 'Invalid ViteHub-managed Skill path.' >&2; exit 1 ;; esac; remainder=$managed; current=${quotedDirectory}; relative=; while [ "$remainder" != "\${remainder#*/}" ]; do component=\${remainder%%/*}; relative=\${relative:+$relative/}$component; current="$current/$component"; if [ -L "$current" ]; then printf '%s\\n' 'ViteHub-managed Skill path cannot traverse a symlink.' >&2; exit 1; fi; if [ ! -e "$current" ]; then encodedParent=$(printf '%s' "$relative" | base64) || exit $?; encodedParent=$(printf '%s' "$encodedParent" | tr -d '\\n') || exit $?; printf '%s\\n' "$encodedParent" >> ${quotedManagedParentsManifest} || exit $?; fi; remainder=\${remainder#*/}; done; if [ ! -e ${quotedDirectory}/"$managed" ] && [ ! -L ${quotedDirectory}/"$managed" ]; then parent=\${managed%/*}; if [ "$parent" != "$managed" ]; then mkdir -p -- ${quotedDirectory}/"$parent" || exit $?; fi; cp -R -- ${quotedStagingDirectory}/"$managed" ${quotedDirectory}/"$managed" || exit $?; printf '%s\\n' "$encoded" >> ${quotedManagedManifest} || exit $?; fi; done && rm -rf -- ${quotedStagingDirectory}`,
  })
