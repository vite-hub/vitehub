...
export function redactCredentialText(value: string): string {
  return value
    ...
    .replace(new RegExp(`${credentialAssignmentPrefix}([^\\s"',;&{}<>]+)`, "gi"), (match, prefix: string, key: string) => isCredentialKey(key) ? `${prefix}[REDACTED]` : match)
}
...
export function pendingCredentialAssignment(value: string): "assignment" | "unquoted" | undefined {
  ...
  if (!assignment || !isCredentialKey(assignment[2]!)) return
...
export function pendingCredentialQuote(value: string): string | undefined {
  ...
  return assignment && isCredentialKey(assignment[2]!) ? assignment[3]?.[0] : undefined
}
