...
import { createMemoryChatStateAdapter } from "../runtime/workspace-state.ts"
import { isChatCapability } from "../capability.ts"
import { createChatBot } from "../runtime/agent-chat.ts"
...
import type { AgentChatRuntimeContext } from "../types.ts"
import type { NitroAgentRuntimeConfig, NitroAgentRuntimeContext } from "../../nitro/handler.ts"

type ChatInput = AgentInput<NitroAgentRuntimeContext>
...
function createMemo(): NitroAgentRuntimeContext["memo"] {
...
function getCloudflareRuntime(event: H3Event): NitroAgentRuntimeContext["cloudflare"] | undefined {
...
function createRuntimeContext(event: H3Event, session: ChatDevtoolsSession): AgentChatRuntimeContext<NitroAgentRuntimeConfig> {
  ...
  } as AgentChatRuntimeContext<NitroAgentRuntimeConfig>
}
...
async function resolveDevtoolsChat(event: H3Event, state: ChatDevtoolsHandlerState, session: ChatDevtoolsSession): Promise<ChatInstance> {
  session.bot ||= (async () => {
    ...
    if (!chatCapability) {
      throw createError({
        statusCode: 500,
        statusMessage: `Agent "${session.name}" does not include chat().`,
      })
    }
    const bot = await createChatBot(agent, {
      ...chatCapability.options,
      adapters: { [chatDevtoolsAdapterName]: adapter },
    }, createRuntimeContext(event, session), createMemoryChatStateAdapter(), session.name)
