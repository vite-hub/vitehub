...
export function cancellableAsyncIterableSource(stream: AsyncIterable<unknown>, options: { deferReader?: boolean } = {}): {
  ...
  const directCancel = isRuntimeRecord(stream)
  ...
  const getIterator = (): AsyncIterator<unknown> => {
    ...
    readableReader = isReadableAsyncIterable(stream) ? stream.getReader() : undefined
  ...
  const cancel = async (reason?: unknown) => {
    ...
    cancelTask ||= settleStreamCancellation(stream, {
      ...
      async return(cancelReason) {
        return await getIterator().return?.(cancelReason) ?? { done: true, value: undefined }
      },
    }, directCancel, reason)
    await cancelTask
  }
