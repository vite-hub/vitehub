...
function generatedWebhookQueueResumeHelper(routeCapabilities: { requestOption: string }): string[] {
  return [
    ...
    `  const stops = Object.entries(webhookHandlers).flatMap(([name, handler]) => typeof handler.resume === 'function' ? [handler.resume({ agentIdentity: agentIdentities[name], ${routeCapabilities.requestOption}webhookState: viteHubChatStateResolver })] : [])`,
