...
async function generateAgentWebhookRouteHandler(
  ...
  return [
    ...
    `import { createAgentChatRouteHandler, createAgentWebhookRouteHandler } from ${JSON.stringify(subpath(agentImportBase, "server"))}`,
    ...
    "const chatHandlers = Object.fromEntries(Object.entries(agents).map(([name, agent]) => [name, createAgentChatRouteHandler(agent, resolveChatRouteOptions(agentModules[name]))]))",
    "const webhookHandlers = Object.fromEntries(Object.entries(agents).map(([name, agent]) => [name, createAgentWebhookRouteHandler(agent)]))",
