...
function withStreamResultCancellation<T extends AsyncIterable<StreamEvent>>(stream: T, result: unknown): T {
  ...
  for (const property of ["stream", "fullStream"] as const) {
    ...
    if (hasRuntimeType(cancel, "function")) {
    ...
    if (!Object.is(iterator, candidate) || !hasRuntimeType(iterator.return, "function")) continue
    Object.defineProperty(stream, Symbol.for("vitehub.agent.stream.cancel"), {
      value: (reason?: unknown) => iterator.return?.(reason),
    })
    break
  }
  return stream
...
function withDrivenStreamResultProperties(
  ...
  const directCancel = cancelSource ?? Reflect.get(stream, Symbol.for("vitehub.agent.stream.cancel"))
...
>(
      ...
      if (shouldPreserveStreamResult || (options.renderOutput
            ...
            toUIMessageStream: {
              ...
              value: (...args: unknown[]) => {
                ...
                const normalizedStream = normalizeUiMessageStream(
                  toReadableAsyncIterableStream(source.stream, { highWaterMark: 0 }),
                )
                const enrichedStream = withEagerStreamUsageExtensions(
                  toReadableAsyncIterableStream(normalizedStream, { highWaterMark: 0 }),
                  invocation,
                  rendered,
                )
                const stream = invocation.runtimeContext.traceLog
                  ? traceUiMessageStream(toReadableAsyncIterableStream(enrichedStream, { highWaterMark: 0 }), invocation)
                  : enrichedStream
                return withReadableStreamCleanup(
                  toReadableAsyncIterableStream(stream, { highWaterMark: 0 }),
                  finishPreserved,
                  {
                    abortSignal: invocation.input.abortSignal,
                    cancelOnAbort: source.cancel,
        ...
        const preserveStream = (renderedStream: AsyncIterable<unknown>) => {
          ...
          const preservedStream = hasRuntimeType((renderedStream as ReadableStream<unknown>).pipeThrough, "function")
            ? toReadableAsyncIterableStream(value, { highWaterMark: 0 })
            : value
          ...
          descriptors.toUIMessageStream = {
            ...
            value: (...args: unknown[]) => {
              ...
              try {
                ...
                const normalizedStream = normalizeUiMessageStream(
                  toReadableAsyncIterableStream(existingStream ?? existingSource?.stream ?? renderedStream, { highWaterMark: 0 }),
                )
                ...
                const tracedStream = existingStream
                  ...
                  : invocation.runtimeContext.traceLog
                    ? traceUiMessageStream(toReadableAsyncIterableStream(streamed!.stream, { highWaterMark: 0 }), invocation)
                    : streamed!.stream
  ...
  return await finalizeAgentInvocationResult(invocation, lifecycle, result, async (result) => {
    ...
    if (options.output === "ui-message-stream") {
      ...
      let uiMessageSource: ReturnType<typeof nonBlockingPendingAsyncIterableSource> | undefined
      const uiMessageSources = new Map<AsyncIterable<unknown>, ReturnType<typeof nonBlockingPendingAsyncIterableSource>>()
        ...
        if (isUIMessageStreamResult(rendered)) {
          ...
          try {
            ...
            for (const property of ["stream", "fullStream", "textStream"] as const) {
              ...
              const candidate = descriptor.value
              if (!isAsyncIterable(candidate)) continue
              uiMessageSources.set(candidate, uiMessageSources.get(candidate) ?? nonBlockingPendingAsyncIterableSource(candidate))
            }
          }
          catch (error) {
            ...
            toUIMessageStream: {
              ...
              value: (...args: unknown[]) => {
                try {
                  ...
                  uiMessageSource = uiMessageSources.get(stream) ?? nonBlockingPendingAsyncIterableSource(stream)
          ...
          uiMessageSource = nonBlockingPendingAsyncIterableSource(rendered)
    ...
    return {
      ...
      value: isAsyncIterable(value)
        ? shouldWrapOutput
          ? withDrivenStreamResultProperties(value, rendered, source?.cancel)
