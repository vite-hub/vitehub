...
const parseStoredSnapshot = (raw: unknown): Snapshot => {
  ...
  if (value.progressBudget !== undefined) parseProgressBudget(value.progressBudget)
...
export class PullRequestInbox {
  ...
  finish(claim: Claim, result: { text: string; retry?: boolean; terminal?: boolean; progress?: ProgressOutcome }): boolean {
    return this.transaction(() => {
      const s = this.get(claim.snapshot.repository, claim.snapshot.number)
      if (!s || s.lease !== claim.token) return false
  ...
  summary(): GitHubInboxSummary[] {
    return this.all().map(s => ({ repository: s.repository, number: s.number, head: s.pr?.head?.sha,
      ...
      dirty: s.generation > s.handled, attempts: s.attempts, nextAt: s.nextAt, lastResult: s.lastResult, progressBudget: s.progressBudget }))
  }
}
