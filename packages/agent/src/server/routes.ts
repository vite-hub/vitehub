    ...
    if (durableDelivery) {
      ...
      try {
        ...
        if (steerKey && steerQueue && !steerLock) {
          ...
          const sameInvoker = !previous?.message?.input || (
            previous.message.settlementStatus === undefined
            && durableSteerInvokerKey(previous.message) === workflowInvokerKey
          )
