...
describe("storage capabilities", () => {
  ...
  it("does not adapt Drizzle-shaped DB handles for raw SQL", async () => {
    const { db } = await import("../src/capabilities.ts")
    const database = {
      db: { run: vi.fn() },
      schema: { notes: true },
    ...
    await expect(tools.db_exec!.execute?.({ rationale: "cleanup", statement: "delete from notes where id = 1" })).rejects.toThrow("raw string execute")
  ...
  it("applies DB SQL guardrails and data/schema permissions", async () => {
    ...
    const readTools = await resolveTools([db()], { db: { databases: { default: database } } })
    ...
    const writeTools = await resolveTools([db({ mode: "write", policy: "allow" })], { db: { databases: { default: database } } })
    ...
    const schemaTools = await resolveTools([db({ policy: "allow", schemaMode: "write" })], { db: { databases: { default: database } } })
