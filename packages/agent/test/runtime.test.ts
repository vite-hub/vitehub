...
describe("agent message protocol", () => {
  ...
  it("accepts channel-local final delivery across multiple message-shaped channels", async () => {
    const { defineAgent } = await import("../src/index.ts")
    const { teams, telegram } = await import("../src/channels.ts")

    expect(() => defineAgent({
      channels: {
        teams: teams({ adapter: () => ({}) as never, messages: { final: "hidden" } }),
        telegram: telegram({ adapter: () => ({}) as never, messages: { final: "message" } }),
      },
      driver: { run: () => "ok" },
    })).not.toThrow()
  })
  ...
  it("rejects channel-local message settings across multiple message-shaped channels", async () => {
    ...
    expect(() => defineAgent({
      ...
      driver: { run: () => "ok" },
    })).toThrow("Channel-local messages options other than commentary, filter, final, or stream are only supported when an Agent defines one message-shaped Channel")
  })
