...
describe("title provider inheritance", () => {
  ...
  it.each([
    { expectedModel: "gpt-main", titleOptions: {} },
    { expectedModel: "gpt-cheap", titleOptions: { model: "gpt-cheap", reasoningEffort: "low" } },
  ])(
    "reuses the normalized provider configuration with model $expectedModel",
    async ({ expectedModel, titleOptions }) => {
      const credentials = vi.fn(() => JSON.stringify({ token: "secret" }));
      const environment = vi.fn(() => ({ CODEX_HOME: "/agent/codex" }));
      const finish = vi.fn();
      ...
      const mainDriver = mocks.adapters.find(
        (adapter) => adapter.model === "gpt-main" && adapter.reasoningEffort === "medium",
      );
      ...
      expect(titleDriver).toMatchObject({
        ...
        permissions: "allow-all",
        provider: "codex",
        reasoningEffort: titleOptions.reasoningEffort ?? "medium",
      });
