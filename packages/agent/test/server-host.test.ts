...
if [ "$1" = "clone" ] && [ -n "$VITEHUB_TEST_CLONE_DELAY" ]; then sleep "$VITEHUB_TEST_CLONE_DELAY"; fi
if [ -n "$VITEHUB_TEST_COMMAND_LOG" ]; then
...
describe("GitHub host", () => {
  ...
  it("verifies the exact pull-request head and removes the temporary checkout", async () => {
    ...
    await expect(readFile(commandLog, "utf8")).resolves.toContain(
      "git clone --no-checkout --no-filter -- https://github.com/vite-hub/vitehub.git",
    )
    await expect(readFile(commandLog, "utf8")).resolves.toContain(
  ...
  it("preserves base access for fork callbacks and uses head access for pushes", async () => {
    ...
    expect(log).toContain("git clone --no-checkout --no-filter -- https://github.com/vite-hub/vitehub.git")
