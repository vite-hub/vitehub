...
describe("Agent Invocations", () => {
  ...
  it.each(["api_token", "password", "secret"].flatMap(key => ["|", ">-", "|2-"].map(indicator => ({ key, indicator }))))("redacts YAML $key scalar $indicator across forced message flushes", async ({ key, indicator }) => {
