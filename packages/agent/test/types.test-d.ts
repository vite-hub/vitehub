...
import { access, blob, browser, chat, chatTitle, db, fetch, getTranscriptionResults, git, inputCommands, kv, mcp, observability, openapi, pullRequestContext, repositoryHost, sandbox, schedule, skills, subagents, transcribe, webSearch, workspaceShell, type SubagentToolInput } from "../src/capabilities.ts"
...
describe("agent public types", () => {
    ...
    defineAgent({
      ...
      hooks: {
        "agent:finish"(event) {
          ...
          expectTypeOf(event.usage).toEqualTypeOf<AgentUsageRecord | undefined>()
  ...
  it("accepts message settings and channels from the Agent Definition", () => {
    ...
    const typedFinishEffect = defineFinishEffect(async (context, event) => {
      ...
      expectTypeOf(context.usage).toEqualTypeOf<AgentUsageRecord | undefined>()
            ...
            return {
              delivery: {
                finishEffects: (context, event) => {
                  ...
                  expectTypeOf(context.context).toEqualTypeOf<AgentChannelDeliveryFinishEffectContext["context"]>()
                  expectTypeOf(context.request).toEqualTypeOf<Request | undefined>()
                  expectTypeOf(context.usage).toEqualTypeOf<AgentChannelDeliveryFinishEffectContext["usage"]>()
