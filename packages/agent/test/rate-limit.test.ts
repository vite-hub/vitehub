...
describe("rateLimit capability", () => {
  ...
  it("uses trusted request invoker identity when configured for invoker", async () => {
    const { defineAgent, runAgent } = await import("../src/index.ts")
    const agent = defineAgent({
      capabilities: [
        rateLimit({
    ...
    await expect(runAgent(agent, runtime(), {
      context: { invoker: { id: "user_1", kind: "user" } },
    })).resolves.toBe("ok")
    await expect(runAgent(agent, runtime(), {
      context: { invoker: { id: "user_2", kind: "user" } },
    })).resolves.toBe("ok")
    await expect(runAgent(agent, runtime(), {
      context: { invoker: { id: "user_1", kind: "user" } },
  ...
  it("uses the selected agent invoker profile by default", async () => {
    const { defineAgent, runAgent } = await import("../src/index.ts")
    const agent = defineAgent({
      capabilities: [
        rateLimit({
      ...
      invoker: {
        profiles: [
          { id: "customer_a", kind: "customer", label: "Customer A", meta: { audience: "buyer" } },
          { id: "customer_b", kind: "customer", label: "Customer B", meta: { audience: "seller" } },
        ],
      },
      run: context => ({
        decision: context.context.get("rate-limit"),
    ...
    await expect(runAgent(agent, runtime(), {
      context: { invokerProfileId: "customer_b" },
    })).resolves.toMatchObject({
      decision: {
        identity: "customer_b",
        identitySource: "invoker",
      ...
      invoker: {
        id: "customer_b",
        meta: { audience: "seller" },
      },
    })
  })

  it("uses an origin-specific fallback invoker when no caller is provided", async () => {
    const { defineAgent, runAgent } = await import("../src/index.ts")
    const agent = defineAgent({
      capabilities: [
        rateLimit({
    ...
    await expect(runAgent(agent, runtime({
      run: { origin: "http", runId: "run_1" },
    }), {})).resolves.toMatchObject({
      decision: {
        identity: "anonymous:http",
        identitySource: "invoker",
      },
      invoker: {
        id: "anonymous:http",
