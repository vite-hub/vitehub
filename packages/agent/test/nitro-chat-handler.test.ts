...
describe("agent Nitro chat routes", () => {
  it("exposes chat.message through an AI SDK-compatible route", async () => {
    ...
    const { defineAgentChatHandler } = await import("../src/nitro/handler.ts")
  ...
  it("requires an agent route param for registry chat handlers", async () => {
    const { defineAgent } = await import("../src/index.ts")
    const { defineAgentChatRegistryHandler } = await import("../src/nitro/handler.ts")
    const agent = defineAgent({
  ...
  it("generates app chat routes from Chat App Exposure", async () => {
    ...
    await expect(readFile(route!.handler, "utf8")).resolves.toContain("@vite-hub/agent/nitro/handler")
