...
ViteHub derives one queue binding per discovered queue. For `server/queues/welcome-email.ts`, the queue name is `welcome-email` and the default binding is `QUEUE_WELCOME_EMAIL`.
...
Set `queue.binding` only when you need a specific binding name.
