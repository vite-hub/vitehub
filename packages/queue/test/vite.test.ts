...
describe("hubQueue", () => {
  ...
  it("infers providers for generated Nitro runtime imports", async () => {
    ...
    const nitroPlugin = await readFile(join(root, ".vitehub", "nitro", "queue", "plugin.ts"), "utf8")
    expect(nitroPlugin).toContain("import * as __vitehubVercelQueue from '@vercel/queue'")
    expect(nitroPlugin).toContain("import { createVercelQueueRuntimeClient } from '@vite-hub/queue/internal/runtime/vercel-client'")
