...
async function runCloudflare(fw: Framework) {
  ...
  try {
    const f: Fetcher = async (path, init) => (await mf.dispatchFetch(`http://localhost${path}`, init as never)).json()
    await assertProbe(f, { hosting: "cloudflare-module", provider: "cloudflare", runtime: "cloudflare" })
...
async function runVercel(fw: Framework) {
  ...
  try {
    ...
    await assertProbe(f, { hosting: "vercel", provider: "vercel", runtime: "vercel" })
    log(`vercel x ${fw} ok`)
  }
  catch (error) {
...
async function runLive(url: string, provider: Provider) {
  ...
  const expected = provider === "cloudflare"
    ? { hosting: "cloudflare-module", provider: "cloudflare", runtime: "cloudflare" }
    : { hosting: "vercel", provider: "vercel", runtime: "vercel" }
