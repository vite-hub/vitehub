  ...
  return [
    ...
    ...(processRuntime
      ? [
          ...
          "  function closeScheduleRuntime() {",
          "    for (const signal of shutdownSignals) nodeProcess?.off(signal, closeScheduleRuntime)",
          "    return runtimeClosing ||= runtimeInstallation.then(async result => {",
          "      if ('controller' in result) await result.controller.close()",
          "    })",
          "  }",
          "  for (const signal of shutdownSignals) nodeProcess?.prependOnceListener(signal, closeScheduleRuntime)",
          "  nitroApp.hooks.hook('close', closeScheduleRuntime)",
        ]
      : []),
    "})",
    "",
