...
describe("Schedule Run bookkeeping", () => {
  ...
  it("keeps static and Runtime Schedule run ids distinct for matching ids", async () => {
    let runtimeCalls = 0
    let staticCalls = 0
    setScheduleRuntimeRegistry({
      report: async () => ({
    ...
    await schedules.create({ cron: "0 9 * * *", id: "schedule-1", target: "report" })
    const scheduledAt = new Date("2026-05-23T09:00:00.000Z")
    const runtimeRun = await schedules.run("schedule-1", { scheduledAt })
    const staticRun = await executeStaticSchedule({
      ...
      definition: {
        ...
        options: { id: "schedule-1" },
      },
      name: "report",
      scheduledAt,
    })

    expect(runtimeRun.id).toBe("srun_runtime_schedule-1_2026-05-23T09:00:00.000Z")
    expect(staticRun.id).toBe("srun_static_schedule-1_2026-05-23T09:00:00.000Z")
