...
import { discoverDatabaseDefinitions, normalizeDBOptions, resolveDBViteConfig } from "../src/config.ts"
...
describe("resolveDBViteConfig", () => {
  ...
  it("resolves named database defaults from definition locations", async () => {
    ...
    expect(resolved?.databases.analytics).toMatchObject({
      connection: { url: "file:.data/db/analytics.sqlite.db" },
      migrationsDir: "server/databases/analytics/migrations",
      mode: "named",
    })
  })

  it("keeps normalizeDBOptions as the default database compatibility shim", async () => {
    const rootDir = await createTempProject()
    await writeDefinition(rootDir, "server/databases/config.ts")

    expect(normalizeDBOptions(undefined, rootDir)).toMatchObject({ name: "default" })
  })
})
