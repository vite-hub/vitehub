import { SourceNotFoundError, UnsourceError } from "./errors.ts"
...
function createMissingContentError(key: string) {
  return new UnsourceError(`[vitehub] Source item ${JSON.stringify(key)} does not provide readable content.`)
}
