...
describe("framework package contract", () => {
  ...
  it("ships every declared export and both CLI names", () => {
    ...
    expect(consolePage).toContain('const paginationRetryKey = computed(() => `${list.cursor.value ?? ""}\\0${paginationRetryRevision.value}`)')
    expect(consolePage).toContain(':retry-key="paginationRetryKey"')
