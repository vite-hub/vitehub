...
describe("framework package contract", () => {
  ...
  it("ships every declared export and both CLI names", () => {
    ...
    expect(consolePage).toContain('params: { agent: agentName, invocation: firstInvocation }')
    expect(consolePage).toContain('router.replace({ name: "vitehub-console-agent", params: { agent: agentName } })')
