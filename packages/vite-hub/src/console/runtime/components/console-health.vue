...
const checkedLabel = computed(() =>
  health.value
    ? new Intl.DateTimeFormat(undefined, {
        ...
        minute: "2-digit",
        second: "2-digit",
      }).format(new Date(health.value.checkedAt))
    : undefined,
);
const readyCount = computed(
  () => health.value?.diagnostics.filter((item) => item.status === "ok").length ?? 0,
);
...
const outcomes = computed(() =>
  health.value
    ? [
        ...
        ...item,
        percent: health.value!.workload.total
          ? Math.round((item.value / health.value!.workload.total) * 100)
          : 0,
      }))
    : [],
);
    ...
    <div class="health-page__scroll">
      <div class="health-page__content">
        ...
        <div v-else-if="error && !health" class="health-notice" data-status="error">
          <div>
            <strong>Health check unavailable</strong><span>{{ error }}</span>
          </div>
        ...
        <template v-else-if="health">
          <div
            v-if="health.status === 'degraded' || error"
            class="health-notice"
            :data-status="health.status === 'degraded' ? 'warning' : 'stale'"
          >
            <UIcon
              :name="health.status === 'degraded' ? 'i-lucide-triangle-alert' : 'i-lucide-wifi-off'"
            />
            <div>
              <strong>{{
              ...
              <span>{{
                error || "The diagnostics below identify the unavailable dependency."
              }}</span>
            </div>
          </div>
          ...
          <section class="health-overview">
            <div class="health-overview__summary">
              <div>
                ...
                <span
                  >{{ readyCount }} of {{ health.diagnostics.length }} checks ready · live
                  service</span
                >
              </div>
              ...
              <dl class="health-services">
                <div
                  v-for="item in primaryDiagnostics"
                  :key="item.label"
                  :data-status="item.status"
                >
                  <dt>
                    <i />{{ item.label }}<small v-if="item.detail">{{ item.detail }}</small>
                  </dt>
                  <dd>{{ item.value }}</dd>
                </div>
              </dl>
            ...
            <div class="health-outcomes">
              ...
              <div class="health-outcomes__plot">
                <div v-for="item in outcomes" :key="item.label" :data-status="item.status">
                  ...
                  <span
                    role="progressbar"
                    :aria-label="`${item.label} invocations`"
                    :aria-valuenow="item.value"
                    :aria-valuemax="health.workload.total"
          ...
          <section class="health-totals">
            <h2>Totals</h2>
            <dl>
              <div v-for="metric in totals" :key="metric.label">
                <dt>{{ metric.label }}</dt>
                <dd>{{ metric.value }}</dd>
              </div>
            </dl>
          ...
          <section class="health-breakdown">
            ...
            <div class="health-breakdown__table" role="table" aria-label="Babysitter diagnostics">
              ...
              <div
                v-for="item in health.diagnostics"
                :key="item.label"
                role="row"
                :data-status="item.status"
              >
