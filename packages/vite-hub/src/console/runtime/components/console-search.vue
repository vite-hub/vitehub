...
async function loadNavigation(discoverAgents = false): Promise<void> {
  ...
  try {
    const sectionsValue = record(
      await requestConsole(props.sectionsBase, { signal: controller.signal }),
    );
    ...
    if (discoverAgents && props.agentNames === undefined && installed.includes("agents")) {
      const agentsValue = record(
        await requestConsole(props.agentsBase, { signal: controller.signal }),
      );
        ...
        <p class="text-xs text-muted">
          {{
            paletteError
              ...
              : value.trim()
                ? "Try a page, Agent, or phrase from a session."
                : "No Console results are available yet."
          }}
        </p>
      </div>
    </template>
