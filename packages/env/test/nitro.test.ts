...
describe("Nitro module", () => {
  ...
  it("types agent runtime runtime config from generated env declarations", async () => {
    ...
    await writeFile(join(root, "typecheck.ts"), [
      ...
      "",
      "type RuntimeConfig = { teams: { apiUrl: string | undefined, appId: string }, vertex: { model: string } }",
      "",
      "defineAgent<RuntimeConfig>({",
      "  workspace: {},",
      "  provider: 'ai-sdk',",
      "  model({ runtimeConfig }: AgentAdapterMetadataContext<RuntimeConfig>) {",
    ...
    await writeFile(join(root, "tsconfig.json"), JSON.stringify({
      compilerOptions: {
        ...
        paths: {
          ...
          "@vitehub/workspace": [join(import.meta.dirname, "../../workspace/dist/index.d.ts")],
        },
        module: "NodeNext",
