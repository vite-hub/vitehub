...
function reclaimDisposableRootProcessesCommand(root: string) {
  return `root=${shellQuote(root)}; owner_uid=$(id -u); references_box_root() { pid=$1; cwd=$(readlink "/proc/$pid/cwd" 2>/dev/null || true); case "$cwd" in "$root"|"$root"/*) return 0 ;; esac; awk -v root="$root" 'BEGIN { RS="\\0" } { value=$0; equals=index(value, "="); if (value == root || index(value, root "/") == 1 || (equals && (substr(value, equals + 1) == root || index(substr(value, equals + 1), root "/") == 1))) found=1 } END { exit found ? 0 : 1 }' "/proc/$pid/cmdline" 2>/dev/null; }; owns_box_process() { pid=$1; status=/proc/$pid/status; test -r "$status" || return 1; ppid=; uid=; while IFS=: read -r key value; do case "$key" in PPid) set -- $value; ppid=$1 ;; Uid) set -- $value; uid=$1 ;; esac; done < "$status"; test "$ppid" = 1 && test "$uid" = "$owner_uid" && references_box_root "$pid"; }; while :; do pids=; for status in /proc/[0-9]*/status; do pid=\${status#/proc/}; pid=\${pid%/status}; if owns_box_process "$pid"; then pids="$pids $pid"; kill -TERM "$pid" 2>/dev/null || true; fi; done; test -n "$pids" || break; sleep 1; for pid in $pids; do owns_box_process "$pid" && kill -KILL "$pid" 2>/dev/null || true; done; done`
}
