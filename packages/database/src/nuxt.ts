...
async function installNitroCloudflareEnvBridge(config: Record<string, unknown>, root: string) {
  ...
  await writeFileIfChanged(resolve(root, generatedNitroDatabaseMiddleware), [
    ...
    "  const mergedEnv = Object.assign({}, vitehubEnv as object, target.req?.runtime?.cloudflare?.env, target.context?._platform?.cloudflare?.env, target.context?.cloudflare?.env, target.env)",
    "  setActiveCloudflareEnv(new Proxy(mergedEnv, { get: (target, property, receiver) => Reflect.has(target, property) ? Reflect.get(target, property, receiver) : Reflect.get(vitehubEnv as object, property) }))",
    "}",
    "",
  ].join("\n"))
}
