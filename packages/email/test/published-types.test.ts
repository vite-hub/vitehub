...
it("publishes the structured Email error contract", async () => {
  const root = await mkdtemp(join(tmpdir(), "vitehub-email-types-"))

  try {
    await cp(fixtureRoot, root, { recursive: true })
    const packDir = join(root, "packs")
    await mkdir(packDir)
    const specs = await packWorkspacePackages(packDir, ["@vite-hub/runtime", "@vite-hub/markdown-template", "@vite-hub/email"])
...
it("keeps Effect internals out of published Email artifacts", async () => {
