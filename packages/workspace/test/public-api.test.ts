...
describe("workspace public API", () => {
  ...
  it("keeps shell as a lazy workspace tools dependency", async () => {
    ...
    expect(packageJson.dependencies?.["@vite-hub/shell"]).toBe("workspace:*")
    expect(packageJson.peerDependencies?.["@vite-hub/shell"]).toBeUndefined()
    expect(packageJson.peerDependenciesMeta?.["@vite-hub/shell"]).toBeUndefined()
