...
Workspace definitions are runtime-free. To run generated code, open a [`@vite-hub/box`](../box/README.md) session and pass it to `workspace.startSession({ host: boxSession })`. Workspace owns diff, commit, and rollback while Box owns execution and the host lifecycle.

Trusted-host Box Sessions expose their local filesystem path, so Workspace automatically projects the transactional Session through MountX when a local transport is available. Commands then operate on the Session directly without an eager host copy, host-tree scans, or a copy-back pass. Remote hosts and machines without a usable MountX transport keep the materialized Session path as a compatibility fallback.
...
The same driver works with MountX's 9P and NFSv4.1 servers for Linux guests, or its S3 gateway for S3-compatible clients. Use `{ readOnly: true }` when the consumer only needs inspection. The native Workspace driver preserves empty directories, special filenames, symlinks, executable metadata, and open-file identity across rename. MountX is still alpha and unaudited, so keep network transports loopback-only unless the surrounding sandbox or network is the explicit security boundary.
