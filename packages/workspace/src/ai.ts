...
function cleanMutationPath(path: string) {
  return cleanWorkspaceMutationPath(path)
}
...
async function runShellCommand(
  ...
  const fs = isWorkspace(input)
    ? createWritableWorkspaceFs(input)
    : createReadonlyWorkspaceFs(input)
  const result = await runWorkspaceInspectionCommand(input, command, {
