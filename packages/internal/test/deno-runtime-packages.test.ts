...
describe("Deno deployment output", () => {
  ...
  it("stages reachable packages and their installed optional native dependencies", async () => {
    ...
    for (const text of ["DENO_DEPLOY_ORG", '["deploy", "create"', "--do-not-use-detected-build-config", "server/index.ts", '["deploy", ".", "--prod"', 'const common = ["--org", organization, "--app", app]', '"--allow-node-modules", ...common', "mkdtemp", "finally"]) expect(deployRunner).toContain(text)
  })
...
`, "utf8")
      ...
      const failedStage = JSON.parse((await readFile(failedInvocationsFile, "utf8")).split("\n")[0]!) as { cwd: string }
