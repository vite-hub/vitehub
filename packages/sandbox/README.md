...
- Vercel Box sessions are closed in a `finally` block after success or failure. Cloudflare also creates and closes a unique Box for each run unless you configure `sandboxId` or pass one to `runSandbox()`. Cleanup errors are suppressed, so a successful return is not an external attestation that provider resources were destroyed.
