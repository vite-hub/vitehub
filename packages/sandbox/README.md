...
- Cloudflare runs with a deterministic Box ID per Definition. Within one runtime isolate, ViteHub serializes concurrent runs that share it; separate Worker isolates can still enter the same Box concurrently. Use external coordination when deployment-wide serialization is required, or give independent runs distinct identities with `runSandbox(name, payload, { sandboxId })`.
