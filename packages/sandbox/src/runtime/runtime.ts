...
const CLOUDFLARE_SANDBOX_RETRY_DELAYS_MS = [1000, 2000, 5000, 10000, 15000]
...
function isRetriableCloudflareSandboxError(error: unknown) {
  ...
  if ((sandboxError?.code || metadata?.code) === 'TIMEOUT')
    return true
...
export async function runSandbox<TPayload = unknown, TResult = unknown>(
  name?: string,
  payload?: TPayload,
  options?: SandboxExecutionOptions,
