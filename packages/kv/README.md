...
Upstash and Deno KV also support `kv.getAndDelete(key)` and `kv.increment(key, ttl)`. These operations are atomic in the provider. Cloudflare KV and `fs-lite` reject them because those backends cannot provide the same guarantee.
