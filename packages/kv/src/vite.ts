...
function serializeCloudflareRuntime(config: ResolvedKVModuleOptions): string {
  return [
    ...
    "    async getAndDelete() { resolveStoreConfig(name); throw new Error(\"[vitehub] Cloudflare KV does not support atomic operations. Use Upstash or Deno KV.\"); },",
    ...
    "    async increment() { resolveStoreConfig(name); throw new Error(\"[vitehub] Cloudflare KV does not support atomic operations. Use Upstash or Deno KV.\"); },",
