...
describe("@vitehub/unshell just-bash runtime", () => {
  ...
  it("runs workspace inspection policy and search through unshell", async () => {
    const workspace = new MemoryWorkspace({
      "README.md": "# Docs\n",
      "models/orders.sql": "select * from orders\n",
    })
...
describe("@vitehub/unshell cloudflare runtime", () => {
  it("delegates to the cloudflare sandbox client", async () => {
    ...
    const result = await runtime.exec("pwd", {
      cwd: "/workspace",
      env: { FOO: "bar" },
      onStderr,
      onStdout,
    ...
    expect(sandbox.exec).toHaveBeenCalledWith("pwd", [], expect.objectContaining({
      cwd: "/workspace",
      env: { FOO: "bar" },
      timeout: 100,
    }))
