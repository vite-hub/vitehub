...
async function run(command: string, args: string[], cwd: string) {
  try {
    ...
    // SAFETY: execFile rejects with an Error carrying optional stdout and stderr diagnostics.
...
async function buildWorker(appDir: string, entry: string, name: string) {
  ...
  return parse(metafileSchema, JSON.parse(await readFile(meta, "utf8")))
}
