...
describe.skipIf(process.env.VITEHUB_CONSUMER_CONTRACT !== "1")("published vite-hub consumer contract", () => {
  it("serves the Console from a packed standalone Nitro app without UI peers", async () => {
    const root = await mkdtemp(join(tmpdir(), "vite-hub-nitro-console-consumer-"))
    ...
    try {
      await Promise.all([
        ...
        mkdir(join(appDir, "server/routes"), { recursive: true }),
      ...
      await Promise.all([
        writeFile(join(appDir, "server/agents/contract.ts"), `
          ...
          export default defineAgent({
            driver: { run: () => "PACKED_NITRO_CONSOLE" },
          })
        `, "utf8"),
        writeFile(join(appDir, "server/routes/index.ts"), `
          export default () => "standalone nitro"
        `, "utf8"),
        writeFile(join(appDir, "vite.config.ts"), `
      ...
      for (const peer of ["@nuxt/ui", "reka-ui", "vue"]) {
        expect(existsSync(join(appDir, "node_modules", ...peer.split("/"))), `${peer} must not be installed at the Nitro app root`).toBe(false)
      }
      await run("pnpm", ["run", "build"], appDir)

      const server = await withNodeServer(join(appDir, ".output/server/index.mjs"), appDir, async (origin) => {
        const [page, script, agents] = await Promise.all([
          fetch(`${origin}/_vitehub`),
          fetch(`${origin}/_vitehub/assets/console.js`),
          fetch(`${origin}/api/_vitehub/console/agents`),
        ])
        return {
          ...
          scriptLength: (await script.arrayBuffer()).byteLength,
          scriptStatus: script.status,
        }
      })

      expect(server.value.pageStatus).toBe(200)
      expect(server.value.page).toContain("/_vitehub/assets/console.js")
      expect(server.value.scriptStatus).toBe(200)
      expect(server.value.scriptContentType).toMatch(/javascript/)
      expect(server.value.scriptLength).toBeGreaterThan(1_000)
      expect(server.value.agentsStatus).toBe(200)
      expect(server.value.agents).toEqual({ agents: ["contract"] })
    }
    finally {
