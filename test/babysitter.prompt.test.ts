...
test('parks CI failures inherited unchanged from the exact base head', async () => {
  ...
  assert.match(prompt, /exact-base run is still pending, queued, or absent/)
  assert.match(prompt, /stop unchanged with `retry`/)
  assert.match(prompt, /cannot wake a parked pass/)
...
test('uses Pullfrog exact-head evidence without blocking on terminal service failure', async () => {
  ...
  assert.match(prompt, /post exactly one comment whose entire body is the following line/)
  assert.match(prompt, /\n@pullfrog Please review this PR at exact head <expected-head> and submit a GitHub review\.\n/)
  assert.match(prompt, /Do not add backticks, code fences, punctuation, or other text to that comment/)
  assert.doesNotMatch(prompt, /`@pullfrog Please review this PR at exact head/)
  assert.match(prompt, /run code-review once as a bounded local fallback against the exact head/)
  assert.match(prompt, /merge when it reports no actionable findings/)
  assert.match(prompt, /Repair its actionable findings under the same one-pass limits/)
