...
`@vite-hub/devtools` is publishable so external applications can install the ViteHub DevTools Integration and test package-owned DevTools Features through the hosted shell.
...
- Putting feature behavior inside `@vite-hub/devtools` was rejected because package-owned runtime state, serialization, and bridges belong to the package that owns the feature.
- A global scanner in `@vite-hub/devtools` was rejected because the shell would need to know how to enable package runtime behavior that belongs to each package integration.
