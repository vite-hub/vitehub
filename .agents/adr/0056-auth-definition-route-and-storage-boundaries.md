...
The package should prefer `server/auth.ts` as the canonical Auth Definition location, support `server.auth.ts` as a singleton alias, and reject duplicate Auth Definition locations. Future Agent integration should bridge Auth User/Auth Session into Agent Invoker without merging the concepts.
