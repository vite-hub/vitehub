...
Input Command handlers may accept by returning nothing, reject with a `Response`, return replacement text, or return a partial Agent run input. Exact parsing, chaining, and merge semantics are implementation details, but the architectural boundary is fixed: Input Commands are explicit pre-model input handling inside the Capability Lifecycle, not a place to store model-facing task instructions. Command-only delivery surfaces should reject unsupported commands at their Channel Delivery Admission boundary instead of changing generic Input Command defaults. Host Commands belong to a later chat/host integration surface.
