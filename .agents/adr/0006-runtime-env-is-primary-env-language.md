...
Use **Runtime Env** for the Env Package concept and **Server Env** for the public server-code API. Generated `#vitehub/env/server` declarations, Nuxt server auto-imports, examples, and documentation expose `ServerEnv` and `useServerEnv(event?: unknown)`.
...
Private bridge code may still mutate Nitro runtime config as the server transport, but bridge names should describe applying Runtime Env rather than applying Env registry values to runtime config.
