...
Package-specific `@vite-hub/*/nitro` modules, generated Nitro plugins, Vite plugin `.nitro` adapters, and Nitro-specific discovery modes are removed except for ADR 0051's Schedule Provider Wake exception, ADR 0052's Workspace Runtime Registry bridge, and ADR 0056's Workspace hosted runtime setup handoff. Server directory discovery belongs to ViteHub's Vite Integration, not to Nitro.
