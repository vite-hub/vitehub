...
- Ambient invocation keys such as `workspaceScope` were rejected as authority; applications must use an explicit `access({ workspace: { resolve } })` resolver when scope comes from request, auth, or the resolved Agent Invoker.
- Generated prompt text from the Selected Workspace Scope was rejected; developers may opt into explicit **Workspace Scope Instructions** instead.
