...
- Ambient invocation keys such as `workspaceScope` were rejected as authority; applications must use an explicit `access({ workspace: { resolve } })` resolver when scope comes from request, auth, or the resolved Agent Actor.
