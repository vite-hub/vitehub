...
The DB Capability consumes an agent-facing DB primitive handle from Capability context. It does not adapt the Drizzle Runtime Surface directly; a Database-owned adapter should provide that handle when `@vite-hub/database` is the backing primitive.
