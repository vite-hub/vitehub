...
ViteHub keeps harness-backed execution in the **Agent Driver** boundary: `defineAgent({ driver: { harness, credentials? }, workspace, capabilities })`. Harness sandbox setup stays Agent Package runtime plumbing behind the **Agent Harness Driver Contract**. The **Agent Package** adapts the harness and owns **Harness Permission Policy**. The **Workspace Package** owns Harness Workspace Session preparation and sync back through Workspace rules. Capabilities remain the place where tools and Skills are enabled, including `.agents/skills/` through `skills()` when a developer opts into that Capability.
