...
ViteHub agents expose model-facing abilities through `defineAgent({ capabilities })`, while KV, Blob, DB, Sandbox, and Workspace remain package-level primitives configured outside the agent. Primitive capabilities such as `kv()`, `blob()`, `db()`, `bash()`, and `sandbox()` can expose those primitives to the model through tools, instructions, policy, and early requirement validation.
...
- Implicit workspace creation for `bash()` and `sandbox()` was rejected because the developer must explicitly choose the filesystem boundary the agent can see.
...
`bash()` exposes curated workspace read/write operations and defaults to read mode. Workspace, Bash, KV, Blob, and DB share `mode: 'read' | 'write'`; write mode enables mutation tools only when the backing primitive and agent workspace allow it. `kv()` and `blob()` expose small read/edit tool surfaces rather than one tool per primitive method; read operations can include scoped prefix discovery, but not unbounded global browsing. `blob()` is direct object storage for generated or uploaded artifacts such as images, audio, video, PDFs, arbitrary binary objects, and object metadata; Workspace remains the default model-facing file-tree boundary for inspectable or editable files. `db()` keeps a separate schema tool and separates data `mode` from `schemaMode` because value mutations and live schema changes are different authorities. Storage write tools require approval by default, but developers can explicitly opt into autonomous storage writes.
