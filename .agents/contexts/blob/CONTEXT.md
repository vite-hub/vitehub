...
- Existing singleton Blob usage targets the **Default Blob Store**.
...
> **Dev:** "Should agents get a Blob capability?"
> **Domain expert:** "No. File/object access for agents goes through Workspace when Workspace is the boundary."
...
- Named Blob APIs were considered as separate exports - resolved: keep the default `blob` API and add store selection such as `blob.store("uploads")`.
