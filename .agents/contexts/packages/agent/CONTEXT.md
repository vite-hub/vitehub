...
- The **Agent Package** does not copy Flue-style root `tools`, `skills`, or `sandbox` fields; custom harness process or session providers use `driver.sandbox`, while default harness sandbox setup stays Agent Package runtime plumbing behind the **Agent Harness Driver Contract**.
