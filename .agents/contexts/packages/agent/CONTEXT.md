...
The Agent Package runtime handler behind the Chat Webhook Route that resolves the Agent's Chat Platforms, invokes the matching platform adapter webhook, and starts the resolved chat Agent Trigger.
...
- A **Chat Webhook Handler** consumes resolved Chat Platform Public Configuration and platform adapters; it does not declare Chat Capability behavior itself.
...
- Generated **Chat Webhook Route** paths live under `/api/_vitehub/agents/:agent/...` so Agent identity stays explicit and package-owned API space is clear.
...
- **Chat Identity Removal** means public Chat Capability configuration does not expose caller identity policy; the Agent Package maps trusted Chat Platform Actor Facts into the default Agent Invoker before Access, Rate Limit, and Agent callbacks consume `context.invoker`.
- **Chat Platform Public Configuration** should use platform language for public API shape, while Chat Platform Adapters remain implementation objects behind each platform.
- A discovered Agent Definition that attaches the Chat Capability is implicitly chat-capable for DevTools and Chat Webhook Route behavior.
...
- App-owned Teams webhook files and public webhook registration helpers were considered for ChatSDK adapters - resolved: the Agent Package owns automatic **Chat Webhook Routes** that consume Chat Platform Public Configuration.
