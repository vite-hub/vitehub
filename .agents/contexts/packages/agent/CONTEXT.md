...
- Public `chat.identity` invocation context was considered for convenience - resolved: remove it because callers should use the resolved Agent Actor, currently exposed as `context.invoker` until `context.actor` lands, or Chat context values instead of a parallel identity string.
