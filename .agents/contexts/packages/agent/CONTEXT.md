...
- The **Agent Package** owns Chat Runtime State and Chat Storage selection.
- The **Agent Package** does not expose Chat SDK state adapters as public API.
- The **Agent Package** can use Workspace and KV package primitives for Chat Storage.

## Example Dialogue

> **Dev:** "Should `@vitehub/agent` let users pass a Chat SDK state adapter?"
> **Domain expert:** "No. The **Agent Package** is the **Chat Runtime Owner** and should expose ViteHub Chat Storage instead."
