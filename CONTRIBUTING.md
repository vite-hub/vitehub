...
If merge verification fails permanently before a tag is created, land the correction on `main`, then manually run **prepare release** on `main` with `recover_failed_release` enabled. That closes stale proposals and lets `uppt` propose the next version. Use the ordinary rerun path for transient failures.
